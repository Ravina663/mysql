...
-- 3. Write column name/s for below written tables in which null values are allowed (schema – sakila). (Hint: use DESC command)
desc Film_category; -- 0/Null
desc city; -- 0/Null
desc Address; -- 0/Null
desc address; -- 0/Null
desc postal_code; -- 0/Null
desc store ;--  0/Null
desc film_actor; -- 0/Null
-- all the above mentioned  contains only null  values

-- 4. Write query to calculate the incline/decline for number of payments made on each date. (schema – sakila, table – payment). Output for result set is shown in figure 3.
